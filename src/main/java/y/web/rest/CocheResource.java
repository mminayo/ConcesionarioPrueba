...
public class CocheResource {
    ...
    public ResponseEntity<List<CocheDTO>> getByColor(@PathVariable String color) {
        log.debug("REST request to get a page of Coches found by their attribute 'color'");
        List<CocheDTO> coches = cocheService.findByColor(color);
        return ResponseEntity.ok().body(coches);
    }

    @GetMapping("/coches/coches-by-color")
